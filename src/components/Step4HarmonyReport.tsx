...
export const Step4HarmonyReport: React.FC<Step4HarmonyReportProps> = ({
  ...
  return (
    <div className="space-y-8 animate-fade-in text-slate-900">
      ...
      <div className="rounded-3xl bg-white p-6 sm:p-8 border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
      ...
      <div className="rounded-3xl bg-white p-6 sm:p-7 border border-slate-200 shadow-sm space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          ...
          <span className="text-[10px] font-mono font-bold uppercase text-slate-700 px-2.5 py-0.5 rounded-full bg-slate-100 border border-slate-200">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-200 text-[10px] font-mono text-slate-500 uppercase">
                <th className="p-4 pl-6 font-bold">Attribute</th>
                <th className="p-4 font-bold">Status</th>
                <th className="p-4 font-bold">Canonical Target</th>
                <th className="p-4 font-bold">Aadhaar (UIDAI)</th>
                <th className="p-4 font-bold">PAN Card (ITD)</th>
                <th className="p-4 pr-6 font-bold">Marksheet / Bank</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 font-mono text-xs">
              {fieldResults.map((result) => (
                <tr key={result.fieldName} className="hover:bg-slate-50/50 transition-colors">
                  <td className="p-4 pl-6 font-sans font-bold text-slate-900">
                    {result.fieldLabel}
                  </td>
                  <td className="p-4">{getStatusBadge(result.status)}</td>
                  <td className="p-4 font-bold text-[#0c2340]">
