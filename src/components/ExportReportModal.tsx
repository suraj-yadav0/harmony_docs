...
export const ExportReportModal: React.FC<ExportReportModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs animate-fade-in text-slate-900">
      <div className="bg-white rounded-3xl w-full max-w-3xl max-h-[90vh] shadow-2xl overflow-y-auto border border-slate-200 flex flex-col">
        
        {/* Modal Header */}
        <div className="flex items-center justify-between p-5 border-b border-slate-200 sticky top-0 bg-white/95 backdrop-blur-sm z-10">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-[#0c2340] text-white flex items-center justify-center">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handlePrint}
              className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-bold bg-[#0c2340] hover:bg-[#16375f] text-white shadow-xs transition-colors cursor-pointer"
            >
        ...
        <div className="p-6 sm:p-8 space-y-6 text-slate-800 printable-area text-xs">
          ...
          <div className="p-5 rounded-2xl bg-slate-50 border border-slate-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              ...
              <h4 className="text-xl font-extrabold text-[#0c2340] mt-0.5">
            ...
            <div className="border border-slate-200 rounded-xl overflow-hidden bg-white">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100 font-mono text-xs">
                  <tr>
                    <td className="p-3 font-sans font-bold text-slate-900">Full Name</td>
                    {uploadedDocs.map((d) => (
                      <td key={d.id} className="p-3 text-slate-800">
                        {d.fields.name?.value || '—'}
                      </td>
                    ))}
                  ...
                  <tr>
                    <td className="p-3 font-sans font-bold text-slate-900">Date of Birth</td>
                    {uploadedDocs.map((d) => (
                      <td key={d.id} className="p-3 text-slate-800">
                        {d.fields.dob?.value || '—'}
                      </td>
                  ...
                  <tr>
                    <td className="p-3 font-sans font-bold text-slate-900">Father&apos;s Name</td>
                    {uploadedDocs.map((d) => (
                      <td key={d.id} className="p-3 text-slate-800">
